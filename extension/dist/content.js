...
const SCRIPT_CONTENT_MAX_LENGTH = 8000000;
function textOf(el) {
...
const LOCATION_NOISE_PATTERN = /(?:\d+\s*[-~]\s*\d+\s*[kK万]|面议|经验|应届|实习|本科|硕士|博士|大专|学历|招聘|发布|更新|招\d+人)/i;
const DESCRIPTION_HINT_PATTERN = /(岗位职责|职位描述|工作内容|任职要求|职位要求|职责|你将负责|我们希望|Job Description|Responsibilities|Requirements|Qualifications|About the job|What you'll do)/i;
const DESCRIPTION_WEAK_PATTERN = /(登录|扫码|分享|举报|收藏|投诉|隐私政策|免责声明|版权所有|推荐企业|公司简介|公司信息|官方微信|立即登录|企业服务热线)/i;
const BOSS_SALARY_PATTERN = /(\d+(?:\.\d+)?\s*(?:-|~|至)\s*\d+(?:\.\d+)?\s*(?:k|K|千|万)(?:[·•]\d+\s*薪)?|面议)/i;
const BOSS_JD_SECTION_START_PATTERN = /(职位描述|岗位职责|工作职责|职位职责|工作内容|你将负责|任职要求|职位要求|任职资格|岗位要求|Job Description|Responsibilities|Requirements|Qualifications)/i;
const BOSS_JD_SECTION_END_PATTERN = /(竞争力分析|公司介绍|公司简介|工商信息|工作地址|职位发布者|去APP|下载APP|立即沟通|举报|分享|收藏|微信扫码|BOSS直聘安全提示|BOSS\s*安全提示|个人综合排名|请立即举报|违法和不良信息举报邮箱)/i;
const BOSS_JD_NOISE_LINE_PATTERN = /(刚刚活跃|今日活跃|本周活跃|半年前活跃|近\d+(?:天|周|月)活跃|去APP与BOSS随时沟通|下载APP|前往APP|立即沟通|分享|举报|收藏|扫码登录|BOSS\s*安全提示|竞争力分析|对搜索结果是否满意|热门职位|热门城市|热门企业|附近城市|企业服务热线|老年人直连热线|工作日\s*8:00\s*-\s*22:00|休息日\s*8:00\s*-\s*22:00|没有更多职位|尝试登录查看全部职位|立即登录|协议与规则|隐私政策|防骗指南|使用帮助|朝阳网警|电子营业执照|京ICP备|算法备案信息|个人综合排名|在人中排名第|加载中|·HR|请立即举报|违法和不良信息举报邮箱|^(?:一般|良好|优秀|极好)$|^[\u4e00-\u9fa5]{1,4}(?:先生|女士)$)/i;
const BOSS_JD_NOISE_FRAGMENT_PATTERN = /(去APP与BOSS随时沟通|前往APP查看|下载APP|立即沟通|分享|举报|收藏|刚刚活跃|今日活跃|本周活跃|微信扫码登录|竞争力分析|对搜索结果是否满意|热门职位|热门城市|热门企业|附近城市|没有更多职位|尝试登录查看全部职位|立即登录|企业服务热线|老年人直连热线|协议与规则|隐私政策|防骗指南|使用帮助|朝阳网警|电子营业执照|京ICP备|算法备案信息|BOSS\s*安全提示|个人综合排名|在人中排名第|加载中|请立即举报|违法和不良信息举报邮箱)/gi;
...
function scoreLocationCandidate(value) {
    ...
    if (text.includes("-") || text.includes("·"))
        score += 1;
    if (LOCATION_NOISE_PATTERN.test(text))
        score -= 7;
    if (text.length > 42)
        score -= 2;
...
function pickLocationByLabel(root) {
...
function appendLdJsonLocation(raw, output) {
    if (!raw)
        return;
    ...
    if (typeof addressLocality === "string")
        output.push(addressLocality);
    const addressRegion = obj.addressRegion;
    if (typeof addressRegion === "string")
        output.push(addressRegion);
    const streetAddress = obj.streetAddress;
    if (typeof streetAddress === "string")
        output.push(streetAddress);
    const address = obj.address;
    if (address)
        appendLdJsonLocation(address, output);
    const graph = obj["@graph"];
    if (graph)
        appendLdJsonLocation(graph, output);
}
function readLdJsonLocationCandidates(root) {
...
function resolveLiepinLocation(pageType, primary, fallback) {
    const candidates = [primary, fallback];
    if (pageType === "detail") {
        candidates.push(pickText(document, [
...
function resolveLocation(platform, pageType, primary, fallback = "") {
...
function scoreDescriptionCandidate(value) {
    ...
    if (lineCount >= 4)
        score += 2;
    if (DESCRIPTION_HINT_PATTERN.test(text))
        score += 7;
    if (DESCRIPTION_WEAK_PATTERN.test(text) && length < 260)
        score -= 5;
    return score;
}
function dedupeDescriptionCandidates(values) {
    const seen = new Set();
    const result = [];
    for (const value of values) {
...
function appendDescriptionFromStructuredData(raw, output, depth = 0) {
    if (!raw || output.length > 80)
        return;
    if (depth > 8)
        return;
    if (Array.isArray(raw)) {
...
function readScriptDescriptionCandidates(root) {
    const scripts = Array.from(root.querySelectorAll("script[type='application/ld+json'], script[type='application/json'], script#__NEXT_DATA__, script#__NUXT_DATA__"));
    if (scripts.length === 0)
        return [];
    const values = [];
    for (const script of scripts) {
        ...
        if (!content)
            continue;
        if (content.length > SCRIPT_CONTENT_MAX_LENGTH)
            continue;
        try {
            const parsed = JSON.parse(content);
...
function readMetaDescriptionCandidates(root) {
    const nodes = Array.from(root.querySelectorAll("meta[name='description'], meta[property='og:description'], meta[name='twitter:description']"));
    return dedupeDescriptionCandidates(nodes
...
function readInlineScriptDescriptionCandidates(root) {
    const scripts = Array.from(root.querySelectorAll("script:not([src])"));
    if (scripts.length === 0)
        return [];
    const values = [];
    const keyDrivenPatterns = [
...
function readSelectorDescriptionCandidates(root, selectors) {
    const values = [];
    for (const selector of selectors) {
        const nodes = root.querySelectorAll(selector);
        nodes.forEach((node) => {
...
function pickBestDescriptionCandidate(values) {
...
function resolveDescriptionFromRoot(root, platform) {
...
function resolveDescription(platform) {
    return resolveDescriptionFromRoot(document, platform);
}
function normalizeBossSalaryCandidate(value) {
    if (!value)
        return "";
...
function resolveBossSalaryText(primary, root) {
    const candidates = [primary];
    candidates.push(...pickAllText(root, [
        ".info-primary .salary",
        ".job-salary",
        ".salary",
...
function sanitizeBossDescription(raw) {
    ...
    const weakNoiseCount = candidate.match(/(对搜索结果是否满意|热门职位|热门城市|热门企业|附近城市|企业服务热线|没有更多职位|立即登录|协议与规则|隐私政策|朝阳网警|京ICP备)/g)?.length ||
        0;
    if (hasSectionHint && cleaned.length >= 60)
        return cleaned;
    if (hasSectionHint && candidate.length >= 80)
        return candidate;
    if (weakNoiseCount === 0 && candidate.length >= 260)
        return candidate;
    return "";
}
const POSTED_AT_HINT_PATTERN = /(datePosted|pubDate|publishedAt|publishTime|publishDate|postedAt|upDate|updateTime|updatedAt|发布时间|更新时间|发布于|更新于|更新|发布)/i;
const POSTED_AT_KEY_HINT_PATTERN = /(datePosted|pubDate|publishedAt|publishTime|publishDate|postedAt|upDate|updateTime|updatedAt)/i;
const POSTED_AT_WEAK_PATTERN = /(登录|扫码|分享|举报|收藏|投诉|隐私政策|免责声明|公司介绍|公司简介|职位描述|岗位职责|任职要求|招聘信息)/i;
function normalizePostedAtCandidate(value) {
    if (!value)
        return "";
    return cleanText(value)
...
function dedupePostedAtCandidates(values) {
    const seen = new Set();
    const result = [];
    for (const value of values) {
...
function coercePostedAtValue(value) {
    if (typeof value === "string") {
        return value;
    }
    if (typeof value === "number" && Number.isFinite(value)) {
        const abs = Math.abs(value);
        const timestampMs = abs > 1000000000000 ? value : abs > 1000000000 ? value * 1000 : NaN;
...
function appendPostedAtFromStructuredData(raw, output, depth = 0) {
    if (!raw || output.length > 60)
        return;
    if (depth > 8)
        return;
    if (Array.isArray(raw)) {
...
function readScriptPostedAtCandidates(root) {
    const scripts = Array.from(root.querySelectorAll("script[type='application/ld+json']"));
    if (scripts.length === 0)
        return [];
    const values = [];
    for (const script of scripts) {
        ...
        if (content.length > SCRIPT_CONTENT_MAX_LENGTH)
            continue;
        if (!POSTED_AT_KEY_HINT_PATTERN.test(content))
            continue;
        try {
            const parsed = JSON.parse(content);
...
function readInlineScriptPostedAtCandidates(root) {
    const scripts = Array.from(root.querySelectorAll("script:not([src])"));
    if (scripts.length === 0)
        return [];
    const values = [];
    const stringPattern = /"(?:datePosted|pubDate|publishedAt|publishTime|publishDate|postedAt|upDate|updateTime|updatedAt)"\s*:\s*"((?:\\.|[^"\\]){4,120})"/g;
    const numberPattern = /"(?:datePosted|pubDate|publishedAt|publishTime|publishDate|postedAt|upDate|updateTime|updatedAt)"\s*:\s*(\d{10,13})/g;
    for (const script of scripts) {
        const content = script.textContent?.trim() || "";
        if (!content)
            continue;
...
function readMetaPostedAtCandidates(root) {
    const nodes = Array.from(root.querySelectorAll("meta[property='article:published_time'], meta[property='article:modified_time'], meta[name='publishdate'], meta[name='pubdate'], meta[name='date'], meta[itemprop='datePosted']"));
    const values = nodes
        .map((node) => node.content || "")
        .filter(Boolean);
    return dedupePostedAtCandidates(values);
}
function scorePostedAtCandidate(value) {
    ...
    if (/20\d{2}[-/.年]\d{1,2}[-/.月]\d{1,2}(?:日)?/.test(text))
        score += 8;
    if (/20\d{2}-\d{1,2}-\d{1,2}T\d{1,2}/.test(text))
        score += 9;
    if (/\d{1,2}月\d{1,2}日/.test(text))
        score += 5;
    if (/发布时间|发布于|更新时间|更新于|更新|发布/.test(text))
        score += 2;
    if (text.length >= 6 && text.length <= 40)
        score += 1;
    if (text.length > 48)
        score -= 6;
    if (POSTED_AT_WEAK_PATTERN.test(text))
        score -= 5;
    return score;
}
function pickBestPostedAtCandidate(values) {
...
function resolvePostedAtFromRoot(platform, root) {
    ...
    if (!fallbackBest || fallbackBest.score < 0)
        return "";
    return fallbackBest.text;
}
function resolvePostedAt(platform) {
...
function parseCompanyFromRecruitText(value) {
...
function resolveZhaopinTitleCompanyFallback() {
    let title = "";
    let company = "";
...
function extractFromDetailPage(platform) {
    ...
    if (!title || !company)
        return null;
    const baseSalary = pickText(document, platform.detailSalary) ||
        listFallbackJob?.salary_text ||
        pickText(document, platform.listSalary);
    const salaryRoot = pickEl(document, [
        ".job-detail-box",
        ".job-card-body",
        ".job-detail",
        ".job-detail-section",
...
function normalizeListCardRoot(platform, candidate) {
    if (platform.source !== "boss")
        return candidate;
...
function getUniqueListCards(platform) {
...
function pickFirstVisibleListCard(platform) {
...
function pickBossActiveListCard(platform) {
    if (platform.source !== "boss")
        return null;
    const active = document.querySelector(".rec-job-list .job-card-wrap.active") ||
...
function resolveCollectJob(platform) {
    if (platform.source === "boss") {
...
async function enrichBossDraftJob(job, platform) {
    if (platform.source !== "boss")
        return job;
    if (job.status !== "draft_pending_jd")
        return job;
    const detailUrl = canonicalUrl(job.apply_url || job.url, window.location.href);
    if (!detailUrl || !/job_detail/i.test(detailUrl))
        return job;
    try {
...
async function enrichCollectedJob(job, platform) {
    const enriched = await enrichBossDraftJob(job, platform);
    return enriched;
}
function collectFromCurrentContext(platform, onDone) {
    if (!platform) {
        onDone({
            ok: false,
...
function ensurePageDrawer() {
  ...
  `;
    ...
    const panel = shell.querySelector(".drawer-panel");
    ...
    function applyPanelPosition(left = panelX, top = panelY) {
    ...
    function placePanelAtDefaultRight() {
        const size = getPanelSize();
    ...
    function ensurePanelVisible() {
        ...
        const fullyOutside = rect.right < PANEL_MARGIN ||
    ...
    const close = () => {
        if (!isOpen && !shell.classList.contains("is-visible"))
            return;
        isOpen = false;
        endDrag();
...
function openDrawer(tab) {
    try {
        ...
        chrome.runtime.sendMessage({ type: "OPEN_DRAWER", tab }, (resp) => {
...
function toggleDrawer(tab = "cart") {
    try {
...
function triggerSyncToServer(onDone) {
...
function createFloatingDock(platform) {
  ...
  `;
    ...
    const toggleGlyphEl = root.querySelector("#offeruFloatingToggleGlyph");
    let panel = null;
    let panelController = null;
    let metaEl = null;
    let collectabilityEl = null;
    let latestMetaText = "草稿 0 条 | 可同步 0 条";
    let latestCollectabilityText = "当前页面：检测中";
    let latestCollectabilityTone = "warn";
    let expanded = false;
    ...
    function getDefaultUndockedLeft() {
    ...
    function resolvePageCollectability(currentPlatform) {
        if (!currentPlatform) {
            return {
                text: "当前页面：暂不支持采集",
                tone: "warn",
    ...
    `;
        ...
        panel = nextPanel;
        metaEl = panel.querySelector("#offeruFloatingMeta");
        collectabilityEl = panel.querySelector("#offeruFloatingCollectability");
        syncPanelSummaryText();
        return nextPanel;
    }
    ...
    function currentPeekTransform() {
        return "translate3d(0, 0, 0)";
    }
    function renderHostTransform() {
    ...
    function dockToEdge(nextSide, left, top) {
        edgeDocked = true;
        setSide(nextSide);
        if (expanded) {
    ...
    function scheduleDockStateSave() {
        if (saveTimer !== null) {
    ...
    function handleDragPointerFinish(event) {
    ...
    function loadThemePreference() {
        chrome.storage.local.get([POPUP_UI_SETTINGS_KEY], (result) => {
            const settings = (result[POPUP_UI_SETTINGS_KEY] || {});
    ...
    function loadShortcutPreference() {
        chrome.storage.local.get([SHORTCUT_SETTINGS_KEY], (result) => {
            const raw = (result[SHORTCUT_SETTINGS_KEY] || {});
            shortcutSettings = {
    ...
    chrome.storage.onChanged.addListener((changes, area) => {
        ...
        if (changes[POPUP_UI_SETTINGS_KEY]) {
            const next = changes[POPUP_UI_SETTINGS_KEY].newValue;
...
function bindPopupCollectBridge() {
    if (popupCollectBridgeBound)
        return;
